...
- **Cutlist generator precision fix:** `buildModules` now derives module W/H/D from **measured 2D wall elevations** (`cad_data.walls_json` cabinets + `photo_elevations.model_json`), not room templates. Elevation modules take precedence (same room+type) over template defaults; every part carries an `elevationSource` trace. Verified: 1847×871×560 measured base → side 871×560, bottom 1811 (=1847−2×18); no 3000mm template leaked.
- **BUG FIXED:** `createOrRefreshCutlist` / `regenerateCutlistParts` / `deleteCutlistModule` called `db.transaction()` but `dbClient` wrapper only exposes `.prepare()` → `/cutlist/refresh` was throwing `db.transaction is not a function`. Added `runInTransaction()` helper using the raw better-sqlite3 handle. Cutlist refresh now works end-to-end.
- Cutlist part dims now precision-clamped (>=10mm, integer) in standards service; accuracy audit credits elevation-derived modules.
