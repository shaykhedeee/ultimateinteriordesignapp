...
export default function CRMLeadDashboard({ onProjectClosed }) {
  ...
  useEffect(() => {
    fetchLeads();
    fetch('http://127.0.0.1:8787/api/diagnostics/api-keys')
      .then(res => res.json())
      .then(setApiKeysInfo)
      .catch(console.error);
  }, []);
  ...
  const fetchLeads = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8787/api/leads');
  ...
  const importDemoLeads = async () => {
    try {
      ...
      await fetch('http://127.0.0.1:8787/api/leads/import', {
        method: 'POST',
  ...
  const triggerCallSimulation = async () => {
    ...
    setAudioWave(true);
    try {
      const res = await fetch(`http://127.0.0.1:8787/api/leads/${selectedLead.id}/call`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const closeDeal = async (leadId, status) => {
    try {
      const res = await fetch(`http://127.0.0.1:8787/api/leads/${leadId}/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 flex gap-3 px-6 pt-4 pb-3 border-b border-slate-800/50 items-center">
        <div className="flex flex-1 gap-3">
          {[
            ...
            { label: 'Deals Closed', value: closedLeads.length, icon: <TrendingUp className="w-4 h-4" />, color: 'text-[var(--gold)]' },
        ...
        <button
          ...
          className={`flex-shrink-0 bg-slate-900 border text-[11px] font-bold px-3 py-2 rounded-xl transition flex items-center gap-1.5 ${
            showKeysPanel ? 'border-[var(--gold)] text-[var(--gold)]' : 'border-slate-800 text-slate-400 hover:text-slate-200'
          }`}
        >
      ...
      {showKeysPanel && apiKeysInfo && (
        <div className="flex-shrink-0 mx-6 mt-3 bg-slate-900/90 border border-[var(--gold)]/30 rounded-2xl p-4 shadow-lg shadow-[var(--gold)]/5 slide-down">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xs font-black text-[var(--gold)] uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div className="flex-shrink-0 p-4 border-b border-slate-800">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-extrabold text-slate-200 tracking-wider uppercase flex items-center gap-2">
                ...
                Inbox Leads Queue
                <span className="text-[9px] bg-[var(--gold)]/10 text-[var(--gold)] border border-[var(--gold)]/20 px-1.5 py-0.5 rounded-full font-bold">{filteredLeads.length}</span>
              </h2>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[var(--gold)]/10 border border-[var(--gold)]/25 hover:bg-[var(--gold)]/20 text-[var(--gold)] text-[10px] font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1 transition"
                >
            ...
            <div className="flex gap-2">
              ...
              <select
                ...
                className="bg-slate-950/60 border border-slate-800 rounded-lg px-2 py-1.5 text-[11px] text-slate-300 outline-none focus:border-[var(--gold)]/40 cursor-pointer"
              >
              ...
              return (
                <div
                  ...
                  className={`lead-card p-3 rounded-xl border cursor-pointer ${
                    selectedLead?.id === lead.id
                      ? 'bg-slate-800/80 border-[var(--gold)]/50 shadow-lg shadow-[var(--gold)]/5'
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2 text-[9px]">
                    ...
                    <span className="text-[var(--gold)]/80 font-semibold font-mono">
        ...
        <div className="flex flex-col gap-4 overflow-hidden">
          ...
          <div className="flex-shrink-0 flex rounded-xl bg-slate-900/80 border border-slate-800 overflow-hidden">
            ...
            <button
              ...
              className={`flex-1 text-[11px] font-bold py-2.5 flex items-center justify-center gap-1.5 transition ${showEmailPanel ? 'bg-[var(--gold)]/15 text-[var(--gold)] border-b-2 border-[var(--gold)]' : 'text-slate-500 hover:text-slate-300'}`}
            >
                ...
                <div className="flex-1 flex flex-col gap-3 overflow-hidden">
                  ...
                  <div className="flex-shrink-0">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950/60 border border-slate-800 rounded-lg px-3 py-2 text-[11px] text-slate-200 outline-none focus:border-[var(--gold)]/40"
                    />
                  </div>
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <textarea
                      ...
                      className="flex-1 w-full bg-slate-950/60 border border-slate-800 rounded-lg px-3 py-2 text-[10px] text-slate-300 font-mono outline-none focus:border-[var(--gold)]/40 resize-none leading-relaxed"
                    />
                  </div>
                  ...
                  <button
                    onClick={sendEmail}
                    className="flex-shrink-0 w-full py-3 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 bg-gradient-to-r from-[var(--gold)] to-[#AA8C2C] hover:from-[#e8c94a] hover:to-[#c4a030] text-slate-950 shadow-lg transition"
                  >
              ...
              <div key={lead.id} className="bg-slate-950 border border-slate-800 p-3.5 rounded-xl flex flex-col gap-2.5 slide-up">
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px]">
                  <div className="bg-slate-900/60 p-2 rounded-lg border border-slate-800">
                    ...
                    <strong className="text-[var(--gold)] font-mono">₹{lead.budget?.toLocaleString()}</strong>
                  </div>
                  <div className="bg-slate-900/60 p-2 rounded-lg border border-slate-800">
                ...
                <div className="flex gap-2 mt-1">
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-[var(--gold)] to-[#AA8C2C] hover:from-[#e8c94a] hover:to-[#c4a030] text-slate-950 font-extrabold text-[10px] uppercase py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow transition"
                  >
