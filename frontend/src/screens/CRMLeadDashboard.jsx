          ...
          <div className="flex-shrink-0 p-4 border-b border-slate-800">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-extrabold text-slate-200 tracking-wider uppercase flex items-center gap-2">
                ...
                Client List
                <span className="text-[9px] bg-[var(--gold)]/10 text-[var(--gold)] border border-[var(--gold)]/20 px-1.5 py-0.5 rounded-full font-bold">{filtered.length}</span>
              </h2>
            </div>
            <div className="flex gap-2">
              ...
              <select value={filterStage} onChange={e => setFilterStage(e.target.value)}
                className="bg-slate-950/60 border border-slate-800 rounded-lg px-2 py-1.5 text-[11px] text-slate-300 outline-none cursor-pointer">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {filtered.length === 0 && (
              <div className="text-center py-12 text-slate-600">
                ...
                <p className="text-[10px] mt-1">Add one or import a CSV</p>
              </div>
            )}
            {filtered.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => setSelectedId(c.id)}
                  className={`p-3 rounded-xl border cursor-pointer transition ${isSel ? 'bg-slate-800/80 border-[var(--gold)]/50 shadow-lg shadow-[var(--gold)]/5' : 'bg-slate-950/60 border-slate-800/80 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <strong className="text-xs font-bold text-slate-200 truncate">{c.name}</strong>
