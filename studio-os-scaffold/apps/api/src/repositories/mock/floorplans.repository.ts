...
export interface FloorPlanVersionRecord {
  ...
  overlay?: {
    ...
    markers: Array<{ id: string; markerType: 'room' | 'module' | 'opening' | 'wall' | 'reference'; x: number; y: number; label: string; color: string; linkedEntityId?: string }>;
    updatedAt: string;
  };
...
export class MockFloorPlansRepository {
  ...
  attachSource(id: UUID, input: { assetName: string; imageDataUrl: string; widthPx: number; heightPx: number; isPdf?: boolean }) {
    const record = this.findById(id);
    if (!record) return null;
    record.source = {
    ...
    if (input.isPdf) {
      // Simulate PDF normalization/rasterization pipeline job
      const jobId = crypto.randomUUID();
      mockStore.jobs.unshift({
        id: jobId,
        jobType: 'pdf_rasterization',
        status: 'succeeded',
        progress: 100,
      ...
      logTimelineEvent(record.projectId, 'floorplan.pdf_rasterized', 'PDF Floorplan Normalized', `Rasterized ${input.assetName} to high-res image canvas`);
    }
  ...
  saveOverlay(
    ...
    if (typeof input.northAngleDeg === 'number') {
      record.northAngleDeg = input.northAngleDeg;
    }
    return record;
  }
