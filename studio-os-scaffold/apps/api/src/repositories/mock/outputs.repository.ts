...
export class MockOutputsRepository {
  ...
  getSuggestedRenderVariants(projectId: UUID, roomRef?: string, variantCount = 3) {
    ...
    // Extract rejected cameras and lighting presets to apply suppression rules
    const feedbackList = mockStore.renderFeedbacks.filter((item) => item.projectId === projectId);
    const rejectedFeedback = feedbackList.filter((item) => item.decision === 'rejected');
    const rejectedCameras = new Set(rejectedFeedback.map((item) => String(item.preferenceSignals?.cameraRef ?? '')));
    const rejectedLighting = new Set(rejectedFeedback.map((item) => String(item.preferenceSignals?.lightingPresetRef ?? '')));
    ...
    return Array.from({ length: variantCount }, (_, index) => {
      const cameraRef = cameras[index] ?? fallbackCameras[index % fallbackCameras.length];
      const lightingPresetRef = lighting[index] ?? fallbackLighting[index % fallbackLighting.length];
      
      let baseScore = 0.92 - index * 0.11 + (roomMemory?.approved ?? 0) * 0.03;
      let penaltyReason = '';
      
      const cameraRejected = rejectedCameras.has(cameraRef);
      const lightingRejected = rejectedLighting.has(lightingPresetRef);
      
      if (cameraRejected && lightingRejected) {
        baseScore -= 0.50;
        penaltyReason = ' [Suppressed: camera + light previously rejected]';
      } else if (cameraRejected) {
        baseScore -= 0.25;
        penaltyReason = ' [Penalized: camera angle rejected]';
      } else if (lightingRejected) {
        baseScore -= 0.20;
        penaltyReason = ' [Penalized: lighting preset rejected]';
      }

      const finalScore = Math.min(0.99, Math.max(0.15, baseScore));
      const statusReason = memory.totalSignals > 0
        ? `Ranked from prior ${memory.totalSignals} signal(s).${penaltyReason}`
        : `Fallback preset recommendation.${penaltyReason}`;
  ...
  rejectRenderVariant(variantId: UUID, note?: string) {
    for (const set of mockStore.renderSets) {
      const variant = set.variants.find((item) => item.id === variantId);
      if (variant) {
        ...
        mockStore.renderFeedbacks.unshift({
          ...
          roomRef: set.roomRef,
          decision: 'rejected',
          note: note ?? 'Rejected by designer/client',
          preferenceSignals: { cameraRef: variant.cameraRef, lightingPresetRef: variant.lightingPresetRef },
          createdAt: new Date().toISOString(),
        });
        logTimelineEvent(set.projectId, 'render_variant.rejected', 'Render variant rejected', `${variant.cameraRef} rejected`);
        return variant;
      }
    }
    return null;
  ...
  shortlistRenderVariant(variantId: UUID) {
    for (const set of mockStore.renderSets) {
      const variant = set.variants.find((item) => item.id === variantId);
      if (variant) {
        variant.approvalStatus = 'shortlisted';
        mockStore.renderFeedbacks.unshift({
          id: crypto.randomUUID(),
          projectId: set.projectId,
          renderSetId: set.id,
          ...
          note: 'Shortlisted by designer/client',
          preferenceSignals: { cameraRef: variant.cameraRef, lightingPresetRef: variant.lightingPresetRef },
          createdAt: new Date().toISOString(),
        });
        logTimelineEvent(set.projectId, 'render_variant.shortlisted', 'Render variant shortlisted', `${variant.cameraRef} shortlisted`);
        return variant;
      }
    }
    return null;
