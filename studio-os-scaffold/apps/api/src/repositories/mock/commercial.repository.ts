...
export class MockCommercialRepository {
  ...
  updatePurchaseOrder(poId: UUID, patch: Partial<PurchaseOrder & { isLocked?: boolean }>) {
    const po = mockStore.purchaseOrders.find((item) => item.id === poId);
    if (!po) return null;
    Object.assign(po, patch);
    logTimelineEvent(po.projectId, 'purchase_order.locked_updated', 'Purchase order lock status changed', `${po.poNumber} lock updated`);
    return po;
  }
}
