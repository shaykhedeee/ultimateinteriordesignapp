...
export class RendersService {
  ...
  async rejectVariant(variantId: UUID, note?: string) {
    return mockOutputsRepository.rejectRenderVariant(variantId, note);
  }

  async shortlistVariant(variantId: UUID) {
    return mockOutputsRepository.shortlistRenderVariant(variantId);
  }
}
