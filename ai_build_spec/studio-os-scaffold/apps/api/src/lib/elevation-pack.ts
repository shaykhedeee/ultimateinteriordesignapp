...
function buildWallElevationSvg({
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalWidth} ${totalHeight}" width="100%" height="100%">
    ...
    <text x="${padding}" y="20" font-size="14" fill="#f4f0e8" font-family="Inter, Arial, sans-serif">${escapeText(room.name)} · ${escapeText(wall.wallId)} · ${side}</text>
    ...
    <g>
      ...
      <text x="${totalWidth * 0.45 + 12}" y="${titleY + 36}" font-size="12" fill="#f4f0e8">${escapeText(room.name)} ${escapeText(wall.wallId)} ${side} elevation</text>
