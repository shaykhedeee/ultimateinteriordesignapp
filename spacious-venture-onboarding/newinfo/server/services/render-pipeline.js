...
class LayoutCompiler {
  ...
  compile(floorPlanAnalysis, options = {}) {
    ...
    for (const roomConstraint of constraints.renderConstraints) {
  ...
  compileRoom(roomConstraint, projectConstraints, style, budget, quality) {
    ...
    const walls = this.buildWallDescriptions(room, components);
  ...
  buildWallDescriptions(room, components) {
    ...
    const westWallComps = components.filter(c =>
      ['tv-unit', 'TV'].includes(c.type || c.moduleType)
    );
    const northWallComps = components.filter(c =>
      ['sofa', 'SOFA'].includes(c.type || c.moduleType)
    );
    const eastWallComps = components.filter(c =>
      c.type === 'DINING_TABLE' || c.type === 'COUNTER'
    );
...
class StructuredPromptCompiler {
  compile(scene) {
    ...
    let prompt = `A photorealistic interior render of a ${scene.project?.style || 'modern'} Indian ${room.roomName || 'living room'}`;
    ...
    prompt += `.\n`;
    prompt += `Quality: Photorealistic, 4K resolution, natural lighting, professional interior photography, warm inviting atmosphere, Indian home decor context with modular furniture, precisely matching the described layout.\n`;
    prompt += `DO NOT add any furniture or elements not described above. Every element must match the description exactly.\n`;
    prompt += `IMPORTANT: Focus on realistic lighting, soft shadows, and authentic Indian interior materials.\n`;
...
class SpatialValidator {
  ...
  checkRoomProportions(scene) {
    const dims = scene.scenes?.dimensions;
  ...
  checkWindowPresence(scene) {
    const walls = scene.scenes?.walls || [];
  ...
  checkComponentPresence(scene) {
    ...
    return {
      ...
      score: totalComponents >= 2 ? Math.min(totalComponents / 5, 1.0) : 0.3,
      weight: 0.35,
      detail: `${totalComponents} components described`
    };
  }
  
  checkLighting(scene) {
    const lighting = scene.scenes?.lighting || [];
    return {
...
module.exports = new RenderPipeline();
