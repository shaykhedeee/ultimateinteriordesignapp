...
export function StudioShell({
  ...
  const clientLine = hasClient
    ...
    : 'No active client';
  const projectCode = project ? `SV-${project.id.slice(0, 6).toUpperCase()}` : 'New intake';
  ...
  function handleSaveDesignerInfo() {
    if (typeof onDesignerInfoChange === 'function') {
      onDesignerInfoChange(editingName.trim(), editingRole.trim());
    }
  }
        ...
        <header className="project-topbar">
          ...
          <div className="topbar-tools">
            ...
            <div className="avatar">{avatarText}</div>
             {isEditingDesigner ? (
              <div className="designer-edit-inline" role="group" aria-label="Edit lead designer">
                <input
                  ref={nameInputRef}
                  value={editingName}
              ...
              <span
                ...
                tabIndex={0}
                role="button"
                aria-label={`${leadDesigner}, ${leadRole}. Click to edit.`}
                title="Click to edit"
              >
                <strong>{leadDesigner}</strong><small>{leadRole}</small>
              </span>
            )}
          </div>
        </header>
