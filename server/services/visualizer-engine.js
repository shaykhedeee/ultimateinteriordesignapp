...
import indianInteriorComponentDetector from './indian-interior-component-detector.js';
...
export async function generateStudioRender(projectId, params) {
  ...
  const safeRoom = String(room).replace(/[^a-zA-Z0-9_-]/g, '_');
  const fileName = `visualizer-${safeRoom}-${id}.png`;
...
export async function editStudioRender(projectId, assetId, params) {
  ...
  const safeRoom = String(room).replace(/[^a-zA-Z0-9_-]/g, '_');
  const fileName = `visualizer-edit-${safeRoom}-${id}.png`;
  const filePath = path.join(storageDir, 'assets', fileName);
  const relativePath = `/storage/assets/${fileName}`;
...
export function getRenderReviews(projectId) {
  const db = getDb();
  return db.prepare('SELECT * FROM render_asset_reviews WHERE project_id = ? ORDER BY updated_at DESC').all(projectId)
    .map(renderReviewRow);
}

export function setRenderReview(projectId, assetId, input = {}) {
  const db = getDb();
  const asset = db.prepare('SELECT * FROM generated_assets WHERE id = ? AND project_id = ?').get(assetId, projectId);
  if (!asset) throw new Error('Render asset not found');
  const allowed = new Set(['approved', 'needs-revision', 'rejected', 'unreviewed']);
  const status = allowed.has(input.status) ? input.status : 'unreviewed';
  const now = new Date().toISOString();
  const existing = db.prepare('SELECT created_at FROM render_asset_reviews WHERE asset_id = ?').get(assetId);
  const review = {
    assetId,
    projectId,
    room: asset.room,
  ...
  db.prepare(`
    INSERT INTO render_asset_reviews
...
export function getApprovedRenderAssets(projectId) {
  const db = getDb();
  return db.prepare(`
    SELECT ga.*, rar.status AS review_status, rar.note AS review_note, rar.updated_at AS reviewed_at
    FROM generated_assets ga
    INNER JOIN render_asset_reviews rar ON rar.asset_id = ga.id
    WHERE ga.project_id = ? AND rar.status = 'approved'
...
export async function generateFastRenderVariants(projectId, params = {}) {
  ...
  const spendMode = params.spendMode || process.env.AI_SPEND_MODE || 'smart-cost';
  const qualityMode = params.qualityMode || params.modelTier || 'standard';
  const maxAuto = spendMode === 'premium-quality' ? 4 : Number(process.env.MAX_AUTO_RENDER_VARIANTS || 2);
  const variantCount = Math.max(1, Math.min(Number(params.variantCount || process.env.DEFAULT_RENDER_VARIANTS || maxAuto), maxAuto, 4));
  ...
  const refinement = await refineRenderPromptWithGemini({
    prompt: renderPlan.prompt,
    room,
  ...
  logRenderGenerationJob(db, {
    projectId,
    room,
    provider: referenceOption ? 'reference-plus-live-provider-chain' : 'live-provider-chain',
    spendMode,
    qualityMode,
    renderPlan,
    reuseMatches,
    requestedVariantCount: Number(params.variantCount || 0),
  ...
  if (referenceOption) {
    variants.push({
      ...referenceOption,
      variantDirection: 'Reference match from saved library for comparison only',
      layoutConstraints: renderPlan.layoutConstraints,
  ...
  return {
    asset: variants.find((item) => !item.isReferenceOption) || variants[0],
    variants,
    renderPlan,
    reuseMatches,
    referenceOption,
...
function logRenderGenerationJob(db, payload) {
  const now = new Date().toISOString();
  db.prepare(`
    INSERT INTO render_generation_jobs
    (id, project_id, room, provider, quality_mode, spend_mode, payload, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    nanoid(12),
    payload.projectId,
    ...
    payload.qualityMode,
    payload.spendMode,
    JSON.stringify({ ...payload, createdAt: now }),
    now
  );
}
...
const vastuRules = {
  kitchen: 'Vastu Guideline: The modular kitchen layout places the cooking gas hob/stove on the South-East counter (Agni) so the user faces East while cooking, and places the single/double bowl sink on the North-East or North-West wet zone counter to separate fire and water elements.',
  living: 'Vastu Guideline: Position the heavy L-shaped sectional sofa and console unit against the South or West walls. Keep the North and East center area of the living room open, spacious, and airy for positive energy circulation.',
  master: 'Vastu Guideline: Place the master bed with the headboard positioned against the South or West wall. Avoid placing heavy wardrobes or high cabinetry along the North or East walls.',
  masterBed: 'Vastu Guideline: Place the master bed with the headboard positioned against the South or West wall. Avoid placing heavy wardrobes or high cabinetry along the North or East walls.',
  pooja: 'Vastu Guideline: Position the pooja mandir cabinet strictly in the North-East corner of the room. Deities should face East or West. Ensure the mandir does not share a wall with any bathroom or toilet.',
  temple: 'Vastu Guideline: Position the pooja mandir cabinet strictly in the North-East corner of the room. Deities should face East or West. Ensure the mandir does not share a wall with any bathroom or toilet.',
  dining: 'Vastu Guideline: Position the dining table in the West or South-East zone, ensuring a comfortable clearance for seating and natural light.',
  crockery: 'Vastu Guideline: Place the crockery display niche in the West or North-West wall niche.'
};

function getOrientationLighting(orientation) {
  const map = {
    East: 'Natural daylighting: Early morning sunlight casting long soft shadows from the East window, 3000K warm morning glow.',
    West: 'Natural daylighting: Golden hour warm beams of low afternoon sun streaming through the West window, creating high texture contrast and soft shadows.',
    North: 'Natural daylighting: Cool diffused natural daylight from the North window, soft ambient shadow illumination, no direct harsh glare.',
    South: 'Natural daylighting: Bright direct natural daylight of mid-day sun from the South window, vibrant ambient illumination.'
  };
...
export function compileFastRenderPlan(project, params, corrections) {
  ...
  const selectedLaminates = matchLaminates(project);
  let laminatePromptInstruction = '';
  if (selectedLaminates && selectedLaminates.length > 0) {
    const listStr = selectedLaminates.map(l => `${l.brand} ${l.name} (${l.finish})`).join(', ');
    laminatePromptInstruction = `Laminates selected for this project are: [${listStr}]. Map these to components: use light white/beige finishes for overhead wall units and ceiling lofts, and warmer woodgrain or dark charcoal finishes for base units and TV console shelves.`;
  }

  const isVastuCompliant = project.vastuCompliant === true || project.vastuCompliant === 'true' || project.vastuCompliant === 1;
  const facingDirection = project.vastuPreferences || project.vastu_direction || 'East';
  let roomOrientation = 'East';
  const directionText = String(facingDirection).toLowerCase();
  if (directionText.includes('west')) roomOrientation = 'West';
  else if (directionText.includes('north')) roomOrientation = 'North';
  else if (directionText.includes('south')) roomOrientation = 'South';
  else if (directionText.includes('east')) roomOrientation = 'East';

  const vastuPrompt = isVastuCompliant ? (vastuRules[room] || '') : '';
  const lightingPrompt = getOrientationLighting(roomOrientation);
  ...
  const prompt = [
    ...
    `Layout accuracy instructions: First convert the annotated 2D floor plan into a materialized layout plan, then render the selected room from that plan. Use manual room zones and component markers as the source of truth for wall placement, camera direction, furniture alignment, and module locations. The result is a realistic prompt-constrained render, not exact CAD output.`,
    layoutConstraints.summary,
    layoutConstraints.placementMap.length ? `Placement map to preserve: ${layoutConstraints.placementMap.join('; ')}.` : '',
    layoutConstraints.cameraDirection ? `Camera instruction: ${layoutConstraints.cameraDirection}.` : '',
    layoutConstraints.materialPlanSummary ? `Materialized 2D plan bridge: ${layoutConstraints.materialPlanSummary}.` : '',
    laminatePromptInstruction,
    vastuPrompt,
    lightingPrompt,
...
function buildLayoutConstraints(project, room, params) {
  ...
  const selectedMarkers = annotations.markers
    ...
    .map((marker) => ({
      ...
      wallSide: wallSideFor(marker),
      viewRole: viewRoleForMarker(marker.type)
    }));
  ...
  const markerText = selectedMarkers.map((marker) => {
    ...
    return `${marker.type} in ${roomLabels(marker.room)} on the ${marker.wallSide} side at x${marker.x} y${marker.y}${notes ? ` (${notes})` : ''}`;
  });
  const placementMap = selectedMarkers.map((marker) => {
    const requirement = [marker.placementNote, marker.furnitureRequirement, marker.sizeNote].filter(Boolean).join(', ');
    return `${marker.type} should read as ${marker.viewRole} on the ${marker.wallSide} side${requirement ? `, ${requirement}` : ''}`;
  });
  const planNotes = [params.floorPlanNotes, project.floorPlanNotes].filter(Boolean).join(' ');
  const cameraDirection = cameraDirectionFor(selectedMarkers, params.cameraAngle);
  const materialPlanSummary = buildMaterialPlanSummary(selectedMarkers, selectedZones, project, params);
  const summary = [
...
function wallSideFor(marker = {}) {
  const x = percent(marker.x);
  const y = percent(marker.y);
  const distances = [
    ['left wall', x],
    ['right wall', 100 - x],
    ['top wall', y],
  ...
  if (distances[0][1] < 18) return distances[0][0];
  if (x < 40) return 'left half';
  if (x > 60) return 'right half';
  if (y < 40) return 'upper half';
  if (y > 60) return 'lower half';
  return 'central zone';
}

function viewRoleForMarker(type = '') {
  const text = String(type).toLowerCase();
  if (text.includes('tv')) return 'main focal elevation';
  if (text.includes('sofa')) return 'seating facing the TV/focal wall';
  if (text.includes('bed')) return 'bed wall and headboard alignment';
  if (text.includes('wardrobe')) return 'full-height storage run';
  if (text.includes('kitchen')) return 'modular kitchen run';
  if (text.includes('island')) return 'central island counter';
  if (text.includes('mandir')) return 'sacred focal niche';
  if (text.includes('dining')) return 'dining and display zone';
  if (text.includes('study')) return 'work desk wall';
  if (text.includes('shoe')) return 'entry storage zone';
  if (text.includes('window')) return 'natural light/opening';
  if (text.includes('door')) return 'entry/opening to preserve';
  return 'marked component';
}

function cameraDirectionFor(markers = [], requestedCamera = '') {
  if (requestedCamera === 'elevation') {
    const focal = markers.find((marker) => /tv|bed|wardrobe|kitchen|mandir/i.test(marker.type));
    return focal ? `straight elevation view facing the ${focal.type} on the ${focal.wallSide}` : 'straight elevation view of the main marked wall';
  }
  const tv = markers.find((marker) => /tv/i.test(marker.type));
  const sofa = markers.find((marker) => /sofa/i.test(marker.type));
  if (tv && sofa) return `camera from sofa side looking toward TV wall; keep TV on ${tv.wallSide} and sofa on ${sofa.wallSide}`;
  const bed = markers.find((marker) => /bed/i.test(marker.type));
  if (bed) return `diagonal bedroom view showing the bed/headboard on the ${bed.wallSide} and storage without changing marked placement`;
  const kitchen = markers.find((marker) => /kitchen/i.test(marker.type));
  if (kitchen) return `wide kitchen view along the ${kitchen.wallSide} run, showing counters, shutters, backsplash, and workflow`;
  const focal = markers.find((marker) => !/door|window/i.test(marker.type));
  return focal ? `three-quarter view anchored on the ${focal.type} at the ${focal.wallSide}` : 'natural wide interior view based on the room zone';
}

function buildMaterialPlanSummary(markers = [], zones = [], project = {}, params = {}) {
  const finish = Array.isArray(project.finishTolerance) ? project.finishTolerance.join(', ') : project.finishTolerance;
  const materials = [
    params.backPanelMaterial ? `TV/back panel material ${params.backPanelMaterial}` : '',
    finish ? `finish tolerance ${finish}` : '',
    project.dislikedMaterials ? `avoid ${project.dislikedMaterials}` : ''
  ].filter(Boolean);
  const componentPlan = markers
    .filter((marker) => !/door|window/i.test(marker.type))
    .slice(0, 6)
    .map((marker) => `${marker.type} assigned to ${marker.wallSide}`)
    .join(', ');
  const zonePlan = zones.map((zone) => `${zone.label} occupies ${Math.round(zone.w)} by ${Math.round(zone.h)} percent of plan`).join(', ');
  return [zonePlan, componentPlan, materials.join(', ')].filter(Boolean).join('; ');
}
...
function keywordTags(value = '') {
  const blocked = new Set([
    'people', 'human', 'humans', 'figures', 'figure', 'silhouettes', 'silhouette',
    'mannequins', 'mannequin', 'pets', 'watermark', 'watermarks', 'logos', 'logo',
    'text', 'unreadable', 'distorted', 'impossible', 'fantasy', 'cluttered'
  ]);
  return String(value)
    .toLowerCase()
    .split(/[^a-z0-9]+/)
