...
import type { MaterialSwapRequest } from './types';

export type MaterialSwapWorkspaceProps = {
  organizationId: string;
  projectId: string;
  zoneId?: string;
  renderId: string;
  renderImageUrl: string;
  onPreview?(payload: Partial<MaterialSwapRequest>): Promise<void>;
  onSubmit?(payload: Partial<MaterialSwapRequest>): Promise<void>;
};

export function MaterialSwapWorkspace(props: MaterialSwapWorkspaceProps) {
    ...
    <div className="grid min-h-[760px] grid-cols-[1.1fr_420px] gap-6 rounded-[24px] border border-white/10 bg-[#0f1116] p-6 text-white shadow-2xl">
      ...
      <aside className="flex h-full flex-col overflow-hidden rounded-[22px] border border-white/10 bg-[#141821]">
        <header className="border-b border-white/10 px-5 py-4">
          <h2 className="text-lg font-semibold tracking-wide">Material Swap Tool</h2>
          <p className="mt-1 text-sm text-white/55">
            Premium workspace scaffold for material-swap. Controls: Target material, current material hint, preserve seams, hardware, reflections, finish blending mode.
          </p>
        </header>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/70">
            ...
            <p className="mt-2">material compatibility, finish realism, seam continuity, and neighboring surface harmony.</p>
          </section>
