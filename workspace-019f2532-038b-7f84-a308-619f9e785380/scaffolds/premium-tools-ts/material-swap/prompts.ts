import type { MaterialSwapAnalysis, MaterialSwapRequest } from './types';

export function buildMaterialSwapPlanSystemPrompt() {
  return [
    'You are AURA, a specialist planner for material swap tool.',
    'You create structured edit plans that preserve geometry, lighting logic, and room consistency.',
    'Focus on material compatibility, finish realism, seam continuity, and neighboring surface harmony.',
...
export function buildMaterialSwapPlanUserPrompt(input: {
  request: MaterialSwapRequest;
  analysis: MaterialSwapAnalysis;
...
export function buildMaterialSwapCritiquePrompt(input: {
  planSummary: string;
  roomContext?: string;
  ...
  return {
    system: [
      'You are AURA Critic for material swap tool.',
