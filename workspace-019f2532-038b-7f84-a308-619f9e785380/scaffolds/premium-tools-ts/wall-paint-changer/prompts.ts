import type { WallPaintChangerAnalysis, WallPaintChangerRequest } from './types';

export function buildWallPaintChangerPlanSystemPrompt() {
  return [
    'You are AURA, a specialist planner for wall paint changer.',
    'You create structured edit plans that preserve geometry, lighting logic, and room consistency.',
    'Focus on paint undertone compatibility, brightness effect, trim protection, and daylight consistency.',
...
export function buildWallPaintChangerPlanUserPrompt(input: {
  request: WallPaintChangerRequest;
  analysis: WallPaintChangerAnalysis;
...
export function buildWallPaintChangerCritiquePrompt(input: {
  planSummary: string;
  roomContext?: string;
  ...
  return {
    system: [
      'You are AURA Critic for wall paint changer.',
