...
function get(pathname){return new Promise((res,rej)=>{http.get('http://127.0.0.1:5055'+pathname,r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>res({code:r.statusCode,body:d}))}).on('error',rej)})}
const j = await get('/api/projects');
const arr = JSON.parse(j.body);
const pid = (arr.find(x=>x.id)||arr[0]||{}).id;
console.log('PID='+pid);
const rf = await new Promise((res,rej)=>{const req=http.request('http://127.0.0.1:5055/api/projects/'+pid+'/cutlist/refresh',{method:'POST'},r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>res({code:r.statusCode,body:d}))});req.end();});
